...
//        let thumbFrame = CGRect(x: bounds.origin.x, y: bounds.origin.y, width: bounds.width / 2, height: bounds.height)
        ...
        // two line in center
        let start = CGPoint(x: bounds.width / 3, y: bounds.height / 3)
        let end = CGPoint(x: bounds.width / 3, y: bounds.height * 2 / 3)
        ...
        let start1 = CGPoint(x: bounds.width * 2 / 3, y: bounds.height / 3)
        let end1 = CGPoint(x: bounds.width * 2 / 3, y: bounds.height * 2 / 3)
        let path1 = UIBezierPath()
        path1.moveToPoint(start1)
        path1.addLineToPoint(end1)
        CGContextSetStrokeColorWithColor(ctx, UIColor.grayColor().CGColor)
        CGContextSetLineWidth(ctx, 0.5)
...
class RangeSlider: UIControl {
    ...
    func updateLayerFrames() {
        ...
        lowerThumbLayer.frame = CGRect(x: lowerThumbCenter, y: 0.0, width: thumbWidth / 2, height: thumbWidth)
        lowerThumbLayer.setNeedsDisplay()

        let upperThumbCenter = CGFloat(positionForValue(upperValue))
        upperThumbLayer.frame = CGRect(x: upperThumbCenter + thumbWidth / 2, y: 0.0, width: thumbWidth / 2, height: thumbWidth)
        upperThumbLayer.setNeedsDisplay()
